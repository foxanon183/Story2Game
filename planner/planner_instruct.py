...
class gptPlanner:
    ...
    def try_expand_syntax(self, syntax):
        ...
        print_no_buffer(result_location,'; ',result_how)
        return ret
    ...
    def get_slots_gpt(self,sentence,context_,last_precondition_type, num_item_state_in_the_plan=0):
        ...
        if slot_flag['how']==True:
            ...
            how_result = self.gpt.gpt_call(context_+get_prompt(how_to_get_to_goal_prompt,"","",' '.join(sentence.split(' ')).replace('.','')), 8, 15, temperature=0.9)
        ...
        if slot_flag['location']==True:
            ...
            for trial in range(4):
                ...
                #print_no_buffer(context_+ get_prompt(parallel_precondition_location_prompt,"",characters[0].strip(),' '+' '.join(sentence.strip().replace('.','').split(' ')[1:])))
                infered_location = self.gpt.gpt_call(context_+get_prompt(parallel_precondition_location_prompt,"",characters[0].strip(),
        ...
        if slot_flag['item']==True:
            ...
            #     print_no_buffer("prompt:",prompt)
            #     while True:
            #         obj_generated = self.gpt.gpt_call(context_+prompt, 8, 10, temperature=0.8,repetition_penalty=1.1)
            #         obj_generated = post_process(obj_generated,separator=',', remove_first_word=False,mode="gen_preconditions", prefix="",freq_threshold=3,simi_threshold=0.85,cutoff_frequency=1)
            ...
            #     cnt+=1
            prompt = get_prompt(parallel_precondition_item_prompt,"",'',
                                                 ' '+' '.join((sentence.replace('.','')+' '+slot['location'][0][1]).strip().split(' ')[1:]))
            print(prompt)
            obj_generated = self.gpt.gpt_call(context_+prompt, 4, 1, temperature=0.8,repetition_penalty=1.0)
            if 'nothing' in obj_generated[0]:
                slot['item needed'] = []
            else:
                slot['item needed'] = [' '+s.strip() for s in obj_generated[0].split('.')[0].split(',')]
        ...
        if slot_flag['who']==True:
            for i in range(3):
                ...
                if slot['interaction with other person']!=[]:
                    print_no_buffer("debug interaction with other person :",slot['interaction with other person'])
                    ...
                    if selected_precondition_type=='item needed' or selected_precondition_type=='location' or selected_precondition_type=='item state':
                        if selected_precondition_type=='item needed':
                            ...
                            gpt_generated_actions = self.gpt.gpt_call(self.context_+get_prompt(action_prompt_obtain_item,' '.join(expanded_sentence.split(' ')[0:]).strip(),character_,preprocess(selected_precondition+'?')),
                                                                    10, 20, temperature=0.9,repetition_penalty=1.0)
    ...
    def pipeline(self,main_quest_line=[],quest_summary="",iterations=5,initial_state=['has money','has gold','has silver'], path='./result'):
        self.context_= ""
        ...
        game_logics,map = visualize_plan(graph,title=self.main_quest_line[-1].replace('.','').strip(),end=self.main_quest_line[-1], path=path)
